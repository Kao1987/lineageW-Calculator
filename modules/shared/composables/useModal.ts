...
export function useModal() {
  ...
  const openModal = (modalId: string, customContent?: string) => {
    ...
    modalState.activeModal = customContent 
...
export const petEvaluateHelpContent = {
  ...
  rating: {
    title: '⭐ 評價等級',
    individual: [
      { level: 'excellent', badge: '頂級', desc: '🌟 頂級（>=1.40倍）：100分 - 成長率達140%以上', score: 100 },
      { level: 'good', badge: '優秀', desc: '⭐ 優秀（1.20-1.39倍）：85分 - 成長率120%-139%', score: 85 },
      { level: 'average', badge: '良好', desc: '✅ 良好（1.00-1.19倍）：70分 - 成長率100%-119%', score: 70 },
      { level: 'normal', badge: '普通', desc: '➡️ 普通（0.85-0.99倍）：55分 - 成長率85%-99%', score: 55 },
      { level: 'poor', badge: '待加強', desc: '⚠️ 待加強（<0.85倍）：30分 - 成長率低於85%', score: 30 },
      { level: 'fixed', badge: '固定值', desc: '🔒 積極性：固定為3，象徵分數70，不參與平均', score: 70 }
    ],
    overall: [
      { level: 'godTier', badge: '神級寵物', desc: '🌟 神級寵物（>=95分）：極品成長，值得重點培養', threshold: 95 },
      { level: 'highQuality', badge: '優質寵物', desc: '⭐ 優質寵物（80-94分）：品質良好，推薦培養', threshold: 80 },
      { level: 'normalPet', badge: '普通寵物', desc: '✅ 普通寵物（60-79分）：合乎預期，可正常使用', threshold: 60 },
      { level: 'needImprovement', badge: '待加強', desc: '⚠️ 待加強（45-59分）：低於平均，需要強化', threshold: 45 },
      { level: 'tragic', badge: '悲劇', desc: '💔 悲劇（<45分）：成長極差，建議重新培養', threshold: 0 }
    ],
    bonusEffects: [
      '忍耐力：每5點 = +1物理防禦',
